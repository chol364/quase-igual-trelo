  ...
  return (
    <main className="relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="hero-grid absolute inset-x-0 top-0 h-[820px] opacity-40" />
      </div>

      <div className="relative mx-auto flex min-h-screen w-full max-w-[1520px] flex-col px-6 py-8 text-white">
        <header className="fade-up mx-auto flex w-full max-w-5xl items-center justify-between gap-4 rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(20,27,44,0.82),rgba(14,20,34,0.72))] px-5 py-4 shadow-[0_30px_80px_rgba(0,0,0,0.26)] backdrop-blur-2xl">
          <div className="flex items-center gap-4">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[linear-gradient(135deg,#22d3ee,#2563eb)] text-sm font-bold shadow-[0_20px_40px_rgba(37,99,235,0.35)]">
              AT
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            <Link className="rounded-full border border-white/10 bg-white/[0.03] px-5 py-2.5 text-sm text-white/78 transition hover:bg-white/[0.08] hover:text-white" href="/login">
              Entrar
            </Link>
            <Link className="rounded-full bg-[linear-gradient(135deg,#3690ff,#2468ff)] px-5 py-2.5 text-sm font-semibold shadow-[0_16px_32px_rgba(36,104,255,0.35)] transition hover:-translate-y-0.5 hover:brightness-110" href="/register">
              Criar conta
            </Link>
          </div>
        </header>
        ...
        <section className="grid flex-1 items-center gap-14 py-14 lg:grid-cols-[1.12fr_0.88fr] lg:py-20">
          <div className="fade-up max-w-3xl">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-3">
              {highlights.map((item, index) => (
                <div
                  key={item.value}
                  className="stagger-rise rounded-[1.6rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-5 backdrop-blur-xl"
          ...
          <div className="fade-up relative" style={{ animationDelay: '120ms' }}>
            <div className="relative overflow-hidden rounded-[2.4rem] border border-white/10 bg-[linear-gradient(180deg,rgba(15,22,37,0.92),rgba(10,15,28,0.88))] p-5 shadow-[0_40px_90px_rgba(0,0,0,0.32)]">
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-3">
                {boardPreview.map((column, index) => (
                  <div
                    key={column.title}
                    className={`stagger-rise rounded-[1.8rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,13,24,0.96),rgba(8,13,24,0.8))] p-4`}
                  ...
                  >
                    ...
                    <div className="mt-4 space-y-3">
                      {column.cards.map((card, cardIndex) => (
                        <div
                          key={card.title}
                          className="hover-lift rounded-[1.35rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-4"
        ...
        <section className="grid gap-6 pb-8 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="fade-up rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(14,20,34,0.9),rgba(11,17,30,0.82))] p-6 shadow-[0_30px_80px_rgba(0,0,0,0.22)]" style={{ animationDelay: '180ms' }}>
            <p className="text-sm uppercase tracking-[0.3em] text-white/38">Capacidades</p>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {featureGrid.map((feature, index) => (
                <div
                  key={feature.title}
                  className="stagger-rise rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5"
          ...
          <div className="fade-up rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(12,19,31,0.92),rgba(10,15,25,0.82))] p-6 shadow-[0_30px_80px_rgba(0,0,0,0.22)]" style={{ animationDelay: '260ms' }}>
            ...
            <div className="mt-8 space-y-4">
              {steps.map((step, index) => (
                <div
                  key={step}
                  className="stagger-rise flex gap-4 rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4"
