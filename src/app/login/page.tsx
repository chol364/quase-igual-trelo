...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-10">
      ...
      <div className="hero-orb-slow absolute left-[42%] top-[18%] h-48 w-48 rounded-full bg-blue-500/10 blur-3xl" />

      <div className="grid w-full max-w-6xl gap-10 lg:grid-cols-[1.08fr_430px]">
        <section className="fade-up ambient-panel rounded-[2.2rem] border border-white/10 bg-[linear-gradient(145deg,rgba(12,24,44,.94),rgba(18,34,63,.82))] p-8 shadow-[0_34px_90px_rgba(0,0,0,0.28)] backdrop-blur xl:p-12">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {points.map((item, index) => (
              <div
                key={item}
                className="stagger-rise spotlight-card rounded-[1.5rem] border border-white/10 bg-white/[0.05] p-5"
        ...
        <div className="fade-up floating-card flex flex-col items-center justify-center gap-5" style={{ animationDelay: '100ms' }}>
          <LoginForm />
